...
import site.remlit.blossom.util.gold
import site.remlit.blossom.util.red
...
object TpaService {
    ...
    fun appendTpa(from: UUID, to: UUID, here: Boolean = false) {
        ...
        if (here) {
            target.sendMessage(gold { "${sender.username} has requested you teleport to them. You can accept with /tpaccept." +
                    " This will expire in $expireTime seconds." })
            sender.sendMessage(gold { "Sending request to teleport to ${target.username}." +
            ...
            target.sendMessage(gold { "${sender.username} has requested to teleport to you. You can accept with /tpaccept." +
                    " This will expire in $expireTime seconds." })
            sender.sendMessage(gold { "Sending request to teleport ${target.username} to you." +
                    " It will expire in 30 seconds." })
        }
    ...
    fun acceptTpa(to: UUID) {
        ...
        pendingList.filter { (k, v) -> v == to }
            .forEach { (k, v) ->
                ...
                if (here) {
                    sender.sendMessage(gold { "${target.username} accepted your teleportation request." })
                    target.sendMessage(gold { "Teleporting to ${sender.username}" +
                ...
                Coroutines.sharedScope.launch {
                    ...
                    if (here) {
                        val senderWorld = Universe.get().getWorld(sender.worldUuid ?: throw GracefulException("Something went wrong"))
                            ?: throw GracefulException("Something went wrong")

                        target.updatePosition(senderWorld, sender.transform, sender.headRotation)
                    } else {
                        val targetWorld = Universe.get().getWorld(target.worldUuid ?: throw GracefulException("Something went wrong"))
                            ?: throw GracefulException("Something went wrong")

                        sender.updatePosition(targetWorld, target.transform, target.headRotation)
                    }
                }
